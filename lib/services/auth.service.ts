...
const API_BASE_URL = ROUTES.API_ROUTE || 'http://localhost:3000';
...
export const authApi = {
	login: async (credentials: LoginRequest): Promise<AuthResponse> => {
		return apiClient.post<AuthResponse>(ROUTES.API_LOGIN, credentials);
	},
	...
	verify: async (token: string): Promise<AuthResponse> => {
		return apiClient.post<AuthResponse>(ROUTES.API_USER_VERIFY, {}, token);
	},

	getProfile: async (token: string): Promise<AuthResponse> => {
		return apiClient.get<AuthResponse>(ROUTES.API_GET_PROFILE, token);
	},

	logout: async (token: string): Promise<AuthResponse> => {
		return apiClient.post<AuthResponse>(ROUTES.API_LOGOUT, {}, token);
	},
};
